...
AS $function$
  SELECT regexp_replace(
    public.unaccent('unaccent'::regdictionary, replace(replace(lower(coalesce(txt, '')), 'ç', 'c'), 'Ç', 'C')),
    '[^a-z0-9]+', ' ', 'g'
  );
$function$;
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE cc.display_name IS NOT NULL
      AND NOT EXISTS (
        ...
        WHERE public.search_norm(pc2.display_name) = public.search_norm(cc.display_name)
      )
  ),
  hay AS (
    SELECT
      ...
      b.category,
      b.image_url,
      public.search_norm(b.display_name) AS n_name,
      public.search_norm(b.brand) AS n_brand,
      (public.search_norm(b.display_name) || ' ' ||
       public.search_norm(b.normalized_name) || ' ' ||
       public.search_norm(b.brand) || ' ' ||
       public.search_norm(b.category)) AS haystack
    FROM base b
  ),
  matched AS (
    SELECT h.*,
  ...
  fuzzy_matches AS (
    ...
    WHERE m.hits = 0
      ...
      AND public.similarity(m.haystack, v_joined) > 0.2
  ),
  combined AS (
