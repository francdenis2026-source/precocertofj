...
UPDATE public.license_plans SET ai_monthly_quota = 0 WHERE slug = 'degustacao';
...
BEGIN
  ...
  -- Planos gratuitos/degustação não têm acesso ao assistente de IA
  IF v_allowed AND (COALESCE(v_limit, 0) <= 0 OR v_plan_slug = 'degustacao') THEN
    v_allowed := false; v_reason := 'plan_not_eligible';
  END IF;
