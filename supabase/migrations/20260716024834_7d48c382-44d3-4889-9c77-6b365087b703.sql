DROP FUNCTION IF EXISTS public.search_catalog_suggestions(text, integer);
...
BEGIN
  ...
  combined AS (
    SELECT sm.*, 0 AS bucket,
           public.similarity(sm.haystack, v_joined)::real AS sim
    FROM strict_matches sm
    UNION ALL
    SELECT fm.*, 1 AS bucket,
           public.similarity(fm.haystack, v_joined)::real AS sim
    FROM fuzzy_matches fm
  )
  SELECT c.id, c.display_name, c.brand, c.category, c.image_url,
         (c.bucket = 1) AS is_fuzzy,
         c.sim          AS similarity
  FROM combined c
  ORDER BY
    c.bucket ASC,
