            ...
            <thead className="sticky top-0 bg-background/95 backdrop-blur">
              <tr className="border-b border-border">
                ...
                {entries.map((e) => (
                  <th
                    key={e.productName}
                    className="min-w-[160px] px-3 py-2 text-left font-mono text-[9px] uppercase tracking-widest text-muted-foreground"
                  >
                ...
                <th className="min-w-[110px] px-3 py-2 text-right font-mono text-[9px] uppercase tracking-widest text-muted-foreground">
                  Total
                </th>
              </tr>
            </thead>
                      ...
                      <td className="px-3 py-2 align-top">
                        <div className="flex items-center gap-1.5">
                          {isBest ? (
                            ...
                            <span className="inline-flex h-4 w-4 items-center justify-center rounded-full bg-muted/60 font-mono text-[9px] font-bold text-muted-foreground">
                              {rowIdx + 1}
                            </span>
                          ) : (
                            <span aria-hidden="true" className="inline-flex h-4 w-4 items-center justify-center rounded-full text-[9px] text-muted-foreground">
                              ·
                            </span>
                          )}
                      ...
                      {cells.map((cell, i) => {
                        ...
                        if (!cell) {
                          return (
                            <td
                              key={i}
                              className="px-3 py-2 align-top font-mono text-[10px] text-muted-foreground"
                            >
                              —
                            </td>
                          );
                        ...
                        return (
                          ...
                          >
                            ...
                            {fresh ? (
                              <p
                                className={
                                  "mt-1 inline-flex items-center gap-1 font-mono text-[9px] uppercase tracking-widest " +
                                  (fresh.stale
                                    ? "text-destructive"
                      ...
                      <td className="px-3 py-2 text-right align-top">
                        {total != null ? (
                          <>
                            ...
                            {isBest ? (
                              <p className="mt-1 font-mono text-[8.5px] uppercase tracking-widest text-accent-strong">
                                melhor total
                              </p>
                            ) : bestTotal != null ? (
          ...
          {(() => {
            ...
            return (
              <div className="mt-2 flex flex-wrap items-center gap-2 rounded-lg border border-primary/20 bg-primary/[0.04] px-3 py-2">
                <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-accent-strong">
                  Melhor combinação
                </span>
                {bestSummary.map((x) => (
                  <span
                    key={x.name}
                    className="inline-flex items-center gap-1 font-mono text-[10px] text-foreground"
                  >
...
export function CompareTray({
  ...
  return (
    ...
    >
      ...
      <span className="inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-primary px-1.5 font-mono text-[10px] font-bold tabular-nums text-primary-foreground">
        {count}
      </span>
      <button
        type="button"
        onClick={onOpen}
        disabled={count < 2}
        className="rounded-full bg-primary px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-widest text-primary-foreground transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
      >
        Ver comparação
      </button>
      <button
        type="button"
        onClick={onClear}
        className="rounded-full border border-border px-2 py-1 font-mono text-[9px] uppercase tracking-widest text-muted-foreground hover:text-foreground"
      >
        Limpar
      </button>
      {count < 2 ? (
