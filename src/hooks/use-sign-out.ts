...
export function useSignOut() {
  ...
  const areaLabel = () => {
    if (typeof window === "undefined") return "da área restrita";
    const p = window.location.pathname;
    if (p.startsWith("/admin")) return "do painel administrativo";
    if (p.startsWith("/colaborador")) return "da área do colaborador";
    if (p.startsWith("/app")) return "do painel do cliente";
    return "da área restrita";
  };
