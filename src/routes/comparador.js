...
import { Plus, Check, Save, Download, FileDown, BellRing, Store as StoreIcon, Trophy, ArrowRight, Lock, Search, SlidersHorizontal, PackageSearch, Scale, LayoutGrid, Rows3, Share2 } from "lucide-react";
...
export const Route = createFileRoute("/comparador")({
    ...
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "Compare preços reais de produtos nos mercados cadastrados. Do mais barato ao mais caro, com imagens e detalhes.",
            },
        ...
        scripts: [
            {
                ...
                children: JSON.stringify({
                    ...
                    description: "Compare preços reais de produtos nos mercados cadastrados, do mais barato ao mais caro.",
...
function formatSize(size_value, size_unit) {
    if (size_value == null)
        return null;
...
function ComparadorPage() {
        ...
        const onKey = (e) => {
            ...
            if (e.key.toLowerCase() === "r") {
                ...
                if (!region)
                    return;
                e.preventDefault();
                const firstFocusable = region.querySelector('[role="link"], a, button, [tabindex]:not([tabindex="-1"])');
    ...
    const sortKey = sortParam === "price-asc" ||
        ...
        ? sortParam
        : "relevance";
    const confFilter = confParam === "alta" || confParam === "media" || confParam === "baixa" ? confParam : "";
    const setQ = (value) => {
    ...
    const setConfFilter = (next) => {
        navigate({ search: (prev) => ({ ...prev, conf: next }) });
    };
    ...
    const allRows = useMemo(() => applyButcherFilter(data ?? [], butcherIds, { requireMinStores: 2 }), [data, butcherIds]);
    ...
    const derivedCacheRef = useRef(new Map());
    const rows = useMemo(() => {
        ...
        if (derivedCacheRef.current.size > 24) {
            ...
            if (firstKey)
                derivedCacheRef.current.delete(firstKey);
        }
    ...
    const handleToggleAlert = async (catalogId) => {
        if (!session) {
    ...
    const equivalentRanking = useMemo(() => {
        if (!q.trim() || rows.length === 0)
            return null;
        const idxs = selectCheapestEquivalentIndexes(rows.map((r) => ({
            name: r.display_name,
            category: r.category,
        ...
        if (members.length === 0)
            return null;
        const cheapestMember = members.reduce((best, row) => Number(row.min_price) < Number(best.min_price) ? row : best, members[0]);
    ...
    const stats = useMemo(() => {
        ...
        for (const r of rows) {
            for (const s of r.stores)
                stores.add(s.establishment_id);
            const price = Number(r.min_price);
            if (!Number.isFinite(price) || price <= 0)
                continue;
            if (!cheapestRow || price < Number(cheapestRow.min_price))
                cheapestRow = r;
        }
    ...
    const confidenceByKey = useMemo(() => {
        const map = new Map();
        const rankOf = (l) => (l === "alta" ? 3 : l === "media" ? 2 : 1);
        for (const r of rows) {
    ...
    const lowQualityCount = useMemo(() => Array.from(confidenceByKey.values()).filter((c) => c.level === "baixa").length, [confidenceByKey]);
    ...
    const sortedRows = useMemo(() => {
        ...
        if (sortKey === "price-asc")
            arr.sort((a, b) => Number(a.min_price) - Number(b.min_price));
        else if (sortKey === "savings-desc")
            arr.sort((a, b) => Number(b.savings_pct) - Number(a.savings_pct));
    ...
    const setSelected = (next) => {
        ...
        navigate({ search: (prev) => ({ ...prev, sel: value }) });
    };
    ...
    const toggleSelect = (key) => {
        if (selected.includes(key)) {
            ...
            if (!compareGate.allow(`compare:${key}`))
                return;
            setSelected([...selected, key]);
        }
    };
    const selectedRows = useMemo(() => selected.map((k) => rows.find((r) => r.product_key === k)).filter(Boolean), [selected, rows]);
        ...
        selectedRows.forEach(r => {
            (r.stores || []).forEach(s => {
                ...
                if (!marketMap.has(key)) {
                    marketMap.set(key, { storeId: s.establishment_id || "none", storeName: s.store_name, total: 0, matchCount: 0, prices: {} });
                }
    ...
    const handleExport = (format) => {
        ...
        if (format === "pdf") {
            ...
            toast.promise(exportComparisonData({
                data: {
                    format: "csv",
                    items: selectedRows.map(r => ({
                        name: r.display_name,
                        prices: Object.fromEntries(rankingsForExport.map(rank => [rank.storeName, rank.prices[r.product_key] || 0]))
                    })),
    ...
    const pagedRows = useMemo(() => sortedRows.slice((page - 1) * RESULTS_PAGE_SIZE, page * RESULTS_PAGE_SIZE), [sortedRows, page, RESULTS_PAGE_SIZE]);
    ...
    const openStoresRow = useMemo(() => {
        if (!productParam)
            return null;
        return rows.find((r) => r.product_key === productParam) ?? null;
    }, [productParam, rows]);
    const setOpenStoresRow = (row) => {
        if (row && user) {
            trackEvent("user_open_comparador_drilldown", { product_key: row.product_key });
        }
        navigate({
            search: (prev) => ({ ...prev, p: row?.product_key ?? "" }),
        });
    };
    const filtersNode = (<>
        {categoryOptions.length > 0 && (<div className="mt-4">
          <QuickFilterBar label="Categoria" ariaLabel="Filtrar por categoria" value={cat || null} onChange={(next) => setCat(next ?? "")} options={[
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <QuickFilterBar label="Ordenar" ariaLabel="Ordenar produtos" value={sortKey} onChange={(next) => setSortKey(next ?? "relevance")} options={[
        ...
        <QuickFilterBar label="Visual" ariaLabel="Modo de exibição" value={view} onChange={(next) => setView(next ?? "grid")} options={[
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        <QuickFilterBar label="Confiança" ariaLabel="Filtrar por nível de confiança dos dados" value={confFilter || null} onChange={(next) => setConfFilter(next ?? "")} options={[
        ...
        {lowQualityCount > 0 && !confFilter && (<button type="button" onClick={() => setConfFilter("baixa")} className="inline-flex items-center gap-1.5 rounded-full border border-destructive/40 bg-destructive/10 px-2.5 py-1 font-mono text-[11px] font-semibold uppercase tracking-wider text-destructive transition hover:bg-destructive/15" title="Ver apenas produtos com dados de baixa qualidade">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-border/40 pt-4">
        <QuickFilterBar label="Disponibilidade" options={[
            { value: "all", label: "Todos" },
            { value: "in_stock", label: "Em estoque", hint: "Apenas itens disponíveis no momento" }
        ]} value={filterAvailability} onChange={(val) => setFilterAvailability(val || "all")}/>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleSaveCart} disabled={isSaving || selected.length === 0} className="h-8 rounded-full border-border bg-background px-3 text-[11px] font-bold uppercase tracking-widest transition hover:border-primary/50">
            {isSaving ? ("Salvando...") : (<>
                <Save className="mr-1.5 h-3.5 w-3.5"/> Salvar Carrinho
              </>)}
          </Button>
          <Button variant="outline" size="sm" onClick={() => handleExport("csv")} className="h-8 rounded-full border-border bg-background px-3 text-[11px] font-bold uppercase tracking-widest transition hover:border-primary/50">
            <Download className="mr-1.5 h-3.5 w-3.5"/> CSV
          </Button>
          <Button variant="outline" size="sm" onClick={() => handleExport("pdf")} className="h-8 rounded-full border-border bg-background px-3 text-[11px] font-bold uppercase tracking-widest transition hover:border-primary/50">
            <FileDown className="mr-1.5 h-3.5 w-3.5"/> PDF
          </Button>
        </div>
      </div>
    </>);
    return (<PageShell fit hideFooter>
      <Nav />
      ...
      <header className="sticky top-0 z-30 border-b border-border/60 bg-background/90 backdrop-blur-md supports-[backdrop-filter]:bg-background/75">
        ...
        <div className="mx-auto grid w-full max-w-7xl grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 py-2.5 md:gap-6 md:px-8 md:py-3">
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={handleShare} className="inline-flex h-7 items-center gap-1.5 rounded-full border border-border px-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground transition-colors hover:border-brand-gold hover:text-[var(--pc-gold-ink)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold" aria-label="Compartilhar esta visualização do comparador">
              <Share2 className="h-3.5 w-3.5" aria-hidden="true"/>
              <span className="hidden sm:inline">Compartilhar</span>
            </button>
            <Link to="/alertas" title="Criar alerta quando o preço cair" className="inline-flex h-7 items-center gap-1.5 rounded-full border border-border px-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground transition-colors hover:border-brand-gold hover:text-[var(--pc-gold-ink)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold">
      ...
      <section className="mx-auto max-w-7xl px-4 pt-2.5 md:px-6 md:pt-8">
        <div className="flex items-center gap-2 rounded-2xl border border-border bg-background px-4 py-2.5 shadow-sm transition focus-within:border-primary focus-within:shadow-[0_0_0_3px_color-mix(in_oklab,var(--color-primary)_25%,transparent)]">
          ...
          <input ref={searchInputRef} type="text" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar produto (ex.: arroz, leite, óleo…)" className="w-full bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground" aria-label="Buscar produto (atalho: barra ou Ctrl+K)" aria-keyshortcuts="/ Control+K"/>
          <kbd aria-hidden className="hidden shrink-0 rounded border border-border bg-card px-1.5 py-0.5 font-mono text-[11px] text-muted-foreground sm:inline-block" title="Atalho: / ou Ctrl+K para buscar, R para resultados">
            /
          </kbd>
          {q && (<button type="button" onClick={() => setQ("")} className="rounded-full border border-border bg-card px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground">
              limpar
            </button>)}
        </div>
        ...
        <div className="mt-3 md:hidden">
          <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
            <SheetTrigger asChild>
              <button type="button" className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-[12.5px] font-bold uppercase tracking-[0.1em] text-foreground transition hover:border-primary/50">
        ...
        {stats.cheapest != null && (<div className="mt-4 md:mt-6">
            <PriceSpotlight kicker="Menor preço agora" productName={stats.cheapestName ?? "Produto em destaque"} price={Number(stats.cheapest)} storeName={stats.cheapestStore} storesAvailable={stats.storeCount} detailSlug={equivalentRanking?.referenceRow?.catalog_slug ?? equivalentRanking?.referenceRow?.display_name ?? null}/>
          </div>)}
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 md:mt-4 md:gap-3">
          ...
          <StatCard label="Mercados com preço" value={String(stats.storeCount)} hint="mercados cadastrados"/>
        </div>

        <PriceAuditAlert report={auditReport}/>

        {equivalentRanking ? (<div className="mt-5">
            <PriceRankingPanel productName={equivalentRanking.brands > 1
                ? `${equivalentRanking.label} · ${equivalentRanking.brands} marcas`
                : equivalentRanking.label} sizeLabel={equivalentRanking.sizeLabel} stores={equivalentRanking.stores} onOpenStore={equivalentRanking.referenceRow && equivalentRanking.brands === 1
      ...
      <section ref={resultsRef} id="resultados" tabIndex={-1} aria-label="Resultados da comparação (atalho: R)" className="mx-auto max-w-7xl px-4 py-5 focus:outline-none md:px-6 md:py-10 focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background">
        ...
        {!isLoading && error && (<ErrorState title="Não foi possível carregar os produtos" message={error.message} onRetry={() => window.location.reload()}/>)}

        {!isLoading && !error && rows.length === 0 && (<EmptyState icon={PackageSearch} title={q ? "Nenhum produto encontrado" : "Ainda não há produtos cadastrados"} message={q
                ? "Tente outro termo de busca ou limpe o filtro."
                : "Cadastre produtos com preço nos mercados para vê-los aqui."} action={q ? (<button type="button" onClick={() => setQ("")} className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-4 py-2 text-xs font-medium text-foreground transition-colors hover:border-primary/40 hover:text-primary">
                  Limpar busca
                </button>) : undefined}/>)}

        {!isLoading && !error && rows.length > 0 && (<div aria-live="polite">
            {view === "grid" ? (<ul className="grid animate-in fade-in gap-2.5 duration-300 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
                {visibleRows.map((row, idx) => (<ProductCard key={row.product_key} row={row} index={idx} imageOverride={row.image_url ? signedImages[row.image_url] : undefined} activeAlerts={activeAlerts} handleToggleAlert={handleToggleAlert} selected={selected.includes(row.product_key)} canSelect={selected.length < MAX_SEL || selected.includes(row.product_key)} onToggleSelect={() => toggleSelect(row.product_key)} onOpenStores={() => setOpenStoresRow(row)}/>))}
              </ul>) : (<ComparisonTable rows={visibleRows} sortKey={sortKey} onSortChange={setSortKey} activeAlerts={activeAlerts} handleToggleAlert={handleToggleAlert} selected={selected} canSelectMore={selected.length < MAX_SEL} onToggleSelect={toggleSelect} onOpenStores={(row) => setOpenStoresRow(row)}/>)}
            {!quota.exceeded && totalPages > 1 && (<nav aria-label="Paginação dos resultados" className="mt-4 flex items-center justify-between gap-3 border-t border-border/60 pt-3">
                <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="inline-flex items-center rounded-full border border-border bg-background px-3.5 py-1.5 text-[12.5px] font-semibold text-foreground transition hover:border-primary/50 disabled:opacity-40">
                  Anterior
                </button>
                ...
                <button type="button" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="inline-flex items-center rounded-full border border-border bg-background px-3.5 py-1.5 text-[12.5px] font-semibold text-foreground transition hover:border-primary/50 disabled:opacity-40">
                  Próxima
                </button>
              </nav>)}
            {quota.exceeded && (<div className="mt-8 pb-24">
                <PaywallInline title={`Você já explorou ${quota.limit} comparações grátis`} subtitle="Crie sua conta grátis para continuar comparando todos os produtos entre os mercados cadastrados — sem limite."/>
              </div>)}
      ...
      {selected.length > 0 && (<div className="pointer-events-none fixed inset-x-0 bottom-4 z-40 flex justify-center px-4">
          <div className="pointer-events-auto flex w-full max-w-xl flex-col gap-3 rounded-2xl border border-border bg-card/95 p-3 shadow-xl backdrop-blur">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setSelected([])} className="rounded-full border border-border bg-background px-3 py-1.5 text-[11.5px] font-semibold text-muted-foreground transition hover:text-foreground">
                  Limpar
                </button>
              </div>
            </div>

            {selected.length >= 2 && (<div className="space-y-3">
                <div className="overflow-hidden rounded-xl border border-border bg-background/50">
                  <div className="overflow-x-auto">
                    ...
                    const finalResults = Array.from(allMarketNames).map(market => {
                        ...
                        selectedRows.forEach(r => {
                            ...
                            if (match) {
                                ...
                                itemsCount++;
                                if (!establishmentId)
                                    establishmentId = match.establishment_id;
                            }
                        });
                    ...
                    return (<>
                            ...
                            return (<tr key={res.market} className={cn(isBest && "bg-savings/5")}>
      ...
      <ProductStoresDialog open={openStoresRow != null} onOpenChange={(next) => {
            if (!next)
                setOpenStoresRow(null);
        }} productName={openStoresRow?.display_name ?? ""} category={openStoresRow?.category ?? null} sizeLabel={openStoresRow
            ? formatSize(openStoresRow.size_value, openStoresRow.size_unit)
            : null} stores={openStoresRow?.stores ?? []} detailSlug={openStoresRow?.catalog_slug ?? openStoresRow?.display_name ?? null}/>

      {sideBySide && (<div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-4xl max-h-[90vh] overflow-y-auto no-scrollbar">
            <SideBySideComparison {...sideBySide} onClose={() => setSideBySide(null)}/>
          </div>
        </div>)}
      ...
      <GuestGateDialog open={compareGate.open} onOpenChange={compareGate.setOpen} action="compare" title="Comparar produtos é grátis para quem tem conta" description="Cadastre-se em 30 segundos (7 dias sem cartão) para montar sua cesta e comparar preços entre mercados sem limite."/>
    </PageShell>);
}
 ...
 */
function ComparisonTable({ rows, sortKey, onSortChange, activeAlerts, handleToggleAlert, selected, canSelectMore, onToggleSelect, onOpenStores, }) {
    const headerBtn = "inline-flex items-center gap-1 rounded font-mono text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground transition hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background";
    const ariaSort = (k) => {
        if (sortKey !== k)
            return "none";
        return k === "price-asc" || k === "name" ? "ascending" : "descending";
    };
    return (<div className="overflow-hidden rounded-2xl border border-border bg-card shadow-[0_1px_2px_color-mix(in_oklab,var(--color-foreground)_8%,transparent)]">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[640px] border-collapse text-left">
          <thead className="sticky top-0 z-10 border-b border-border bg-card/95 backdrop-blur">
            <tr>
              ...
              <th scope="col" aria-sort={ariaSort("savings-desc")} className="px-4 py-3 text-right">
                <button type="button" className={headerBtn} onClick={() => onSortChange("savings-desc")}>
                  Economia
                </button>
              </th>
          ...
          <tbody className="divide-y divide-border">
            {rows.map((row, idx) => (<ComparisonTableRow key={row.product_key} row={row} index={idx} activeAlerts={activeAlerts} handleToggleAlert={handleToggleAlert} selected={selected.includes(row.product_key)} canSelect={canSelectMore || selected.includes(row.product_key)} onToggleSelect={() => onToggleSelect(row.product_key)} onOpenStores={() => onOpenStores(row)}/>))}
          </tbody>
        </table>
      </div>
    </div>);
}
function ComparisonTableRow({ row, index, activeAlerts, handleToggleAlert, selected, canSelect, onToggleSelect, onOpenStores, }) {
    const { locked } = useTeaserAccess(row.product_key, index);
    const isMulti = Number(row.store_count) > 1;
    if (locked) {
        return (<tr className="bg-card/60">
        <td colSpan={6} className="px-4 py-3">
          <div className="flex items-center justify-between gap-3 rounded-lg border border-dashed border-primary/25 bg-primary/5 px-3 py-2">
            ...
            <Link to="/login" className="rounded-full border border-primary/30 bg-primary/10 px-3 py-1 font-mono text-[11px] font-bold uppercase tracking-widest text-primary transition hover:bg-primary/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background">
              Desbloquear
            </Link>
          </div>
        </td>
    ...
    return (<tr className={"pc-row-interactive group transition-colors " +
            (selected ? "bg-primary/[0.06]" : "")}>
      <td className="px-4 py-3">
        <button type="button" onClick={onOpenStores} className="line-clamp-2 text-left font-display text-[13.5px] font-semibold leading-snug text-foreground transition hover:text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-card rounded" aria-label={`Ver preços de ${row.display_name} em todos os estabelecimentos`}>
          {row.display_name}
        </button>
        {row.category && (<p className="mt-0.5 text-[11px] uppercase tracking-widest text-muted-foreground">
            {row.category}
          </p>)}
        <button type="button" onClick={(e) => {
            e.stopPropagation();
            handleToggleAlert(row.product_key);
        }} className={cn("mt-2 flex h-7 w-7 items-center justify-center rounded-full border shadow-sm transition-all", activeAlerts.includes(row.product_key)
            ? "border-primary bg-primary text-primary-foreground"
            : "border-border bg-background/90 text-muted-foreground hover:text-primary")} title={activeAlerts.includes(row.product_key) ? "Remover alerta" : "Ativar alerta de preço"}>
      ...
      <td className="pc-best-result pc-best-result--compact px-4 py-3 text-right border-l border-[color-mix(in_oklab,var(--pc-gold-ink)_20%,transparent)] bg-[color-mix(in_oklab,var(--pc-gold-ink)_5%,transparent)]" aria-label="Menor preço do produto">
        ...
        <div className="mt-1 flex flex-wrap items-center justify-end gap-1">
          <UnitPriceBadge price={Number(row.min_price)} productName={row.display_name} sizeValue={row.size_value} sizeUnit={row.size_unit}/>
          {(() => {
            const c = computeConfidence({
      ...
      <td className="hidden px-4 py-3 text-right md:table-cell">
        {isMulti ? (<span className="inline-flex flex-col items-end gap-0.5">
      ...
      <td className="px-4 py-3 text-right">
        {isMulti && Number(row.savings_pct) > 0 ? (<SavingsBadge pct={Number(row.savings_pct)} variant="tonal" size="sm" precision={1}/>) : (<span className="text-xs text-muted-foreground">—</span>)}
      </td>
      ...
      <td className="px-4 py-3 text-right">
        <button type="button" onClick={onToggleSelect} disabled={!canSelect && !selected} aria-pressed={selected} aria-label={selected
            ? `Remover ${row.display_name} da comparação`
            : `Adicionar ${row.display_name} à comparação`} className={"inline-flex items-center gap-1 rounded-full border px-2.5 py-1 font-mono text-[11px] font-bold uppercase tracking-widest transition focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-card " +
            (selected
                ? "border-primary bg-primary text-primary-foreground"
                : canSelect
...
function StatCard({ label, value, hint }) {
    return (<div className="rounded-xl border border-border bg-card p-2.5 md:rounded-2xl md:p-5">
...
function ProductCardBase({ row, index, imageOverride, activeAlerts, handleToggleAlert, selected, canSelect, onToggleSelect, onOpenStores, }) {
    const size = formatSize(row.size_value, row.size_unit);
    const stores = Array.isArray(row.stores) ? row.stores : [];
    const isMulti = Number(row.store_count) > 1;
    return (<li className="cv-card relative h-full">
      <TeaserCard id={row.product_key} index={index} variant="full" reason="Este card mostra menor preço, média e ranking de mercados — dados exclusivos para contas cadastradas. Entre grátis para comparar e abrir o drilldown por estabelecimento." trackEventName="visitor_click_unlock_comparador" trackPayload={{ product_key: row.product_key, rank: index + 1 }}>
      <button type="button" onClick={onToggleSelect} disabled={!canSelect && !selected} aria-pressed={selected} aria-label={selected
            ? `Remover ${row.display_name} da comparação`
            : `Adicionar ${row.display_name} à comparação`} className={"absolute left-2 top-2 z-20 inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-bold uppercase tracking-widest shadow-sm backdrop-blur transition " +
            (selected
                ? "border-primary bg-primary text-primary-foreground"
                : canSelect
      ...
      <button type="button" onClick={onOpenStores} aria-label={`Ver preços de ${row.display_name} em todos os estabelecimentos`} className={"hairline-gold group relative flex h-full w-full flex-col overflow-hidden rounded-xl border bg-card text-left shadow-[0_1px_2px_color-mix(in_oklab,var(--color-foreground)_8%,transparent)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_28px_-18px_color-mix(in_oklab,var(--color-primary)_35%,transparent)] focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background " +
            (selected
                ? "border-primary ring-2 ring-primary/25"
                : "border-border hover:border-accent/50")}>
        {/* Editorial ordinal */}
        <span aria-hidden className="pointer-events-none absolute right-2.5 top-2.5 z-10 font-display text-[11px] italic tracking-tight text-accent">
          Nº {String(index + 1).padStart(2, "0")}
        </span>

        {/* Media */}
        <div className="relative aspect-[3/2] w-full overflow-hidden bg-gradient-to-br from-muted/60 to-background">
          <ProductImage src={imageOverride ?? row.image_url} alt={row.display_name} className="absolute inset-0 h-full w-full" imageClassName="h-full w-full" fit="contain" loading="lazy" width={320} sizes="(min-width: 1536px) 16vw, (min-width: 1280px) 20vw, (min-width: 1024px) 25vw, (min-width: 640px) 33vw, 50vw"/>
          {isMulti && Number(row.savings_pct) > 0 && (<div className="absolute left-1.5 top-8">
          ...
          <button type="button" onClick={(e) => {
            e.stopPropagation();
            handleToggleAlert(row.product_key);
        }} className={cn("absolute left-1.5 bottom-1.5 flex h-7 w-7 items-center justify-center rounded-full border shadow-sm transition-all", activeAlerts.includes(row.product_key)
            ? "border-primary bg-primary text-primary-foreground"
            : "border-border bg-background/90 text-muted-foreground hover:text-primary")} title={activeAlerts.includes(row.product_key) ? "Remover alerta" : "Ativar alerta de preço"}>
            <BellRing className="h-3.5 w-3.5" strokeWidth={2.5}/>
          </button>
        ...
        <div className="mt-1.5 border-y border-accent/25 bg-background/40 px-2.5 py-2 sm:px-3">
          ...
          <PriceHero minPrice={Number(row.min_price)} avgPrice={Number(row.avg_price)} savingsPct={Number(row.savings_pct)} cheapestStore={row.cheapest_store} isMulti={isMulti} size="sm"/>
          {isMulti && (<p className="mt-1 truncate text-[11px] leading-tight text-muted-foreground">
              Maior no município:{" "}
              <Price value={Number(row.max_price)} size="sm" tone="muted"/>
        ...
        <ul className="flex-1 divide-y divide-accent/15">
          {Array.from({ length: 2 }).map((_, idx) => {
            ...
            const isBest = idx === 0;
            if (!s) {
                return (<li key={`empty-${idx}`} className="flex h-7 items-center px-2.5 sm:h-8 sm:px-3" aria-hidden>
                  <span className="text-[11px] italic text-muted-foreground/50">—</span>
                </li>);
            }
            return (<li key={s.establishment_id} className={"flex h-7 items-center justify-between gap-2 px-2.5 sm:h-8 sm:px-3 " +
                    (isBest ? "bg-savings/[0.06]" : "")}>
                <div className="flex min-w-0 items-center gap-1.5">
                  {isBest ? (<Trophy className="h-2.5 w-2.5 shrink-0 text-accent" strokeWidth={2.25}/>) : (<span className="h-1 w-1 shrink-0 rounded-full bg-accent/40"/>)}
                  <span className={cn("truncate leading-none", tc.storeNameTight)} title={s.store_name}>
                    {shortenStoreName(s.store_name)}
                  </span>
                </div>
                <Price value={Number(s.price)} size="sm" tone={isBest ? "savings" : "muted"} className="shrink-0"/>
